...
export const HIVE_MAP: readonly HiveComponent[] = [
  ...
  component({
    ...
    gap: "V1 constitutional kernel (findings, dispositions, the §6/§7 defensive ladder, fabric detectors) plus the V2 Shield+Guard architecture built under DEC-027 (Sentinel_World_Class_Security_Architecture_V2, sha256 60d7ddb1…): chamber cross-coverage with authority never assumed and evidence-gated restoration; five security condition levels where tightening is pre-approved and RELAXING always needs Governance; the 8-rung operational action ladder with least-destructive selection and bounded (TTL-or-rollback) containment records that Sentinel requests and never executes; containment succeeding even when event publication fails; the 10-step immune protocol with Guard-independent verification and forensic-preservation-before-destructive-cleanup; the deny-by-default handshake gate where a null check denies exactly like false; Guard modules (trust freshness fails closed on expiry, integrity tamper evidence, policy drift detection with remediation pointed at PolicyIQ/Governance, role-separated supply-chain thresholds with deployment authority not-sentinel); Shield modules (ATT&CK-tagged findings with authority none, fusion that never confirms by volume, §21.4 AI defense — no identity no capability, prompt injection structurally inert against control metadata, AI recommendations never executable, behavior envelopes, deception assets that never carry real data); operator protection (the last recovery authority is irremovable — the operation does not exist; recovery-authority quarantine downgrades to session pause; break-glass on separate credentials); the §21.8 upgrade chain with §21.9 Bootstrap Governance (bootstrap satisfies quorum only in the recorded state, marked for review; exit criteria; re-entry is break-glass + constitutional record); telemetry secret-screening that refuses rather than redacts; the Collective gate (raw incident content never promotable); the §16 scorecard where unevidenced dimensions never score. The 21 specialist candidates are MODULES pending individual chartering (DEC-027 point 2). **Security Observation Plane added under DEC-028 (directive §9-§13):** the canonical SecurityObservation record whose single admission door refuses secret material by naming the field (never redact-and-hope) and refuses protected-health/regulated data from ever being collective-eligible; Hive security semantic conventions with a guard keeping vendor names out of the canonical vocabulary; provider-neutral sensor ports where an unbound sensor is a NAMED blind spot and coverage is a set difference, never a percentage; twelve Hive-native translators (Governance refusal, Fabric route denied/failed, trust degradation, integrity + Sentinel-self mismatch, audit-chain failure capped at probable, AI envelope excursion, prompt injection that never inlines the attacker's directives, cross-tenant attempt, break-glass, chamber health) that take severity/confidence from the owning component and invent neither. **Increments 2-5 (DEC-028) added:** six detection methods where coverage travels with every finding, a baseline below its minimum is indeterminate rather than normal, and gateEligible is computed so an AI-proposed detection can never gate; the exposure graph whose reachability is a LOWER BOUND with no 'unreachable' verdict in the vocabulary; incident correlation on witnessed relationships only (never time proximity) where severity rises but never falls without a named human, and closing requires recovery evidence plus the authority review; SOC read models whose partial-coverage qualification lives in the field name; condition postures that are monotone across all level pairs with no elapsed-time path back to a lower level; containment executor ports where an unbound executor is a refusal (a believed-applied containment is worse than none), dispatch is idempotent, and a failed release keeps the containment visible as standing; forensic reconstruction of references only with declared causality and marked-not-dropped unverifiable elements; self-check where an unattested surface is degraded and a mismatch always escalates with authorityChange none; supply-chain gating on distinct signing roles where an SBOM is evidence, not a requirement. STILL NOT: any bound sensor (every detector evaluates supplied observations), any bound executor (nothing has ever been contained), verification that a component actually applied a posture, the Digital Twin/ATT&CK corpus so detection coverage is unmeasured, host adapters, compliance profiles, persistence, events. Certification evidence: Documents/ProWorks-Ecosystem/projects/sentinel/SENTINEL-CERTIFICATION-EVIDENCE.md — certified at kernel scope, explicitly NOT certified for deployment. Deliberately takes no Governance dependency (§8, §15).",
  }),
