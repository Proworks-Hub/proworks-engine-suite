...
export const HIVE_MAP: readonly HiveComponent[] = [
  ...
  component({
    ...
    gap: "Phases A-B (capture and diagnose): scenario harness, sandbox boundary, fault injection, evidence recorder, failure signature, invariant classifier, causal graph, diagnostic pipeline and a read-only diagnostic bot. Nothing repairs, validates or learns yet. Only five of the catalog's 26 invariants have detectors, so most assessments return NOT_ASSESSED — visible, and deliberately not HELD. The harness drives no engines — a host supplies the executor and the condition evaluators, so an unevaluated corpus condition counts as unheld rather than passed. Most corpus fault classes are constitutional (SOURCE_OF_TRUTH_THEFT) and have no mechanical injection form, so those scenarios run INCONCLUSIVE until a host can produce the fault.",
  }),
