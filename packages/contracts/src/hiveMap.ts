...
export const HIVE_MAP: readonly HiveComponent[] = [
  ...
  component({
    ...
    gap: "Phases A-C (capture, diagnose, repair): scenario harness, sandbox boundary, fault injection, evidence recorder, failure signature, invariant classifier, causal graph, diagnostic pipeline, read-only diagnostic bot, repair candidate model with structural forbidden-shortcut rejection, expiring agent leases and isolated candidate workspaces. Nothing validates or learns yet, and no RepairBot authors a candidate — the model exists, the author does not. Only five of the catalog's 26 invariants have detectors, so most assessments return NOT_ASSESSED — visible, and deliberately not HELD. A scenario's own prose prohibitions cannot be structurally matched and are returned for human review rather than pretend-decided. The harness drives no engines — a host supplies the executor and the condition evaluators, so an unevaluated corpus condition counts as unheld rather than passed. Most corpus fault classes are constitutional (SOURCE_OF_TRUTH_THEFT) and have no mechanical injection form, so those scenarios run INCONCLUSIVE until a host can produce the fault.",
  }),
